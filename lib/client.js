...
class Client {
  ...
  send(header, body, opts = {}) {
    return new Promise((resolve, reject) => {
      // Handle timeouts
      let timeout = setTimeout(() => reject(new Utils.ErrorWith("Request has timed out", { header: header, body: body })), 10*1000) // TODO make timeout configurable and move into own handler
  ...
  _send(header, body, callback = null) {
    ...
    if (callback) {
      msg.setHeader(Tags.RequestId, this._requestId);
    }
    
    // Add request to waitingToSend queue and incremement requestId
    this._waitingToSend[this._requestId] = {
      req: msg,
      callback: callback,
      time: new Date(),
    };
  ...
  _onData(data) {
    ...
    let reply = this._waitingForReply[msg.getHeader(Tags.RequestId)];
    if (reply) {
      reply.callback(msg);
      delete this._waitingForReply[msg.getHeader(Tags.RequestId)];
    }
  }
  ...
  _processWaiting() { 
    ...
    if (!this.socket.pending) {
      ...
      for (let requestId in this._waitingToSend) {
        let msg = this._waitingToSend[requestId];
