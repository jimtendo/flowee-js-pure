...
class TransactionService extends Service {
  ...
  */
  async getTransaction(txId) {  
    ...
    let indexerReply = await this.instance.getIndexer().send({ [Tags.ServiceId]: 19, [Tags.MessageId]: 2 }, {
      4: txId
    });
  ...
  */
  async sendTransaction(transaction) {
    let reply = await this.instance.getHub().send({ [Tags.ServiceId]: 2, [Tags.MessageId]: 2}, {
      20: transaction
    });
