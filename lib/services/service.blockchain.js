...
class BlockchainService extends Service {
  ...
  async getBlockChainInfo() {
    let reply = await this.instance.send({ [Tags.ServiceId]: 1, [Tags.MessageId]: 0 });
  ...
  async getBestBlockHash(body) {
    let reply = await this.instance.send({ [Tags.ServiceId]: 1, [Tags.MessageId]: 2 });
  ...
  async getBlockCount(body) {
    let reply = await this.instance.send({ [Tags.ServiceId]: 1, [Tags.MessageId]: 10 });
