...
class BlockNotificationService extends Service {
  ...
  subscribe(callback) {
    ...
    if (!Object.keys(this.callbacks).length) {
      this.instance.getHub().sendOnly({ [Tags.ServiceId]: 18, [Tags.MessageId]: 0 });
    }
  ...
  unsubscribe(callbackId) { 
    ...
    if (!Object.keys(this.callbacks).length) {
      this.instance.getHub().sendOnly({ [Tags.ServiceId]: 18, [Tags.MessageId]: 2 });
    }
  }
  ...
  _handleNewBlockOnChain(msg) {
    this.instance.getHub().addHandler({ [Tags.ServiceId]: 18, [Tags.MessageId]: 4 }, (msg) => {      
